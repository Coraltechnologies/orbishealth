...
class Common {
	...
	function clinician_list($venue_id = null){
		...
		if(isset($venue_id) && $venue_id !=''){
				...
				$ci->db->select('UserPersonalDetail.user_id,UserPersonalDetail.title,UserPersonalDetail.firstname,UserPersonalDetail.lastname,Role.role,User.status,UserProfessionalDetail.qualifications');
				$query 																		= $ci->db->get();
	...
	function check_user_type($user_id){
		...
		if($user_id){
			$ci				 =& get_instance();
			$condition		 = array('User.id'=>$user_id,'User.status'=>'active');
			$ci->db->select('User.id,User.user_type');
			$ci->db->from('users as User');			
			$ci->db->where($condition);
			$query			= $ci->db->get();
			if($query->num_rows() > 0){
