...
class Common {
	...
	function get_roles($user_id = null){
		if($user_id){
			$ci 								=& get_instance();
			$conditions							= array('UserRoles.user_id'=>$user_id);
			$ci->db->order_by("role","asc");
			$ci->db->where($conditions);
		...
		if(isset($venue_id) && $venue_id !=''){
				...
				if(!empty($query->result_array())){
					...
					foreach($query->result_array() as $key => $value){
						...
						$result['response_data'][$value['user_id']]['img_path']				= $value['img_path'];
						$result['response_data'][$value['user_id']]['img_name']		        = $value['img_name'];
					}
				}else{
					$result['response']														= 'empty';
				}
